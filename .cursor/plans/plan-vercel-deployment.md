...
- Immediate next action: `/review-plan` hardening pass, then `/execute`
...
- [ ] 🟥 Step 1: Code fixes for Vercel
  - [ ] 🟥 1.1: `src/app/api/reminders/route.ts` — add `GET` export sharing the exact bearer-secret check (secret-only; no signed-in-user path on GET), keep POST unchanged; add `export const runtime = "nodejs"`
...
- [ ] 🟥 Step 5: [user] Vercel — create project from the GitHub repo; enter all env vars from Config / Environment; deploy. Agent provides the checklist; user reports the deployed URL
